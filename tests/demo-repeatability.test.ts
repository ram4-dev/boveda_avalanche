...
const paymentPayload = {
  ...
  externalPaymentRef: 'wire-pay-repeat-001'
};
...
describe('demo repeatability after reset', () => {
  it('supports two consecutive runs without stale payment/liquidation evidence', async () => {
    ...
    const runOneEvents = await app.inject({ method: 'GET', url: '/events?loanId=loan-web3-001' });
    ...
    const afterResetEvents = await app.inject({ method: 'GET', url: '/events?loanId=loan-web3-001' });
    ...
    const runTwoEvents = await app.inject({ method: 'GET', url: '/events?loanId=loan-web3-001' });
